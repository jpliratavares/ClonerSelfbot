...
(async function () {
	setTitle("JPLTDEV | Setup");
	console.log(" ");
	console.log(" ");
	console.log(center(`

_________ _______  _    _________ ______   _______          
\__    _/(  ____ )( \   \__   __/(  __  \ (  ____ \|\     /|
   ...
   |  |  | (      | |      | |   | |   ) || (       \ \_/ / 
|\_)  )  | )      | (____/\| |   | (__/  )| (____/\  \   /  
(____/   |/       (_______/)_(   (______/ (_______/   \_/   `.blue, 112));
...
console.log(" ");
console.log(" ");
let token = await question(('- Insira o token:'.cyan));
let copy = await question(('- Insira o ID do server q quer copiar:'.cyan));
let paste = await question(('- Insira o ID do server que quer colar o servidor:'.cyan));
...
client.on("ready", async() => {
	try {
		...
		log.info(`Logado como ${client.user.tag} ( ${client.user.email})`)
	} catch (e) {
		log.error(e.stack)
	}


	setTitle("JPLTDEV | Clonando Servidor");
  ...
  let region = allowedRegions.includes(guild2.region) ? guild2.region : 'eu-central';
	...
	let guildname = guild1.name + " - jpltDEV";
